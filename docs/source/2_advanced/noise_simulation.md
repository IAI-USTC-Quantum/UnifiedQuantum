...
本页是 [本地模拟](../1_basic_usage/simulation.md) 的延伸阅读，适合已经完成基础模拟验证、需要更真实模拟环境的读者。

> 如果你还未完成基础线路构建与无噪声模拟，建议先阅读 [本地模拟](../1_basic_usage/simulation.md)。
...
```python
from uniqc.simulator.error_model import BitFlip, Depolarizing, TwoQubitDepolarizing
...
```python
from uniqc.simulator.error_model import ErrorLoader_GateTypeError, Depolarizing, AmplitudeDamping
