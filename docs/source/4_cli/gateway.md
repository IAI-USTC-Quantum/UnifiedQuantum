...
`uniqc gateway` 子命令用于在后台启动一个本地 [Web UI](../5_webui/index.md) 服务，提供基于浏览器的任务、后端、配置查看入口。它读取 `~/.uniqc/config.yaml` 中的 `gateway` 段（如 `host`、`port`），并把进程信息写入 `~/.uniqc/cache/gateway.pid`（日志为 `~/.uniqc/cache/gateway.log`），方便后续 `stop`/`status` 操作。
...
[`~/.uniqc/config.yaml`](config.md) 的 `gateway` 段中可配置：
