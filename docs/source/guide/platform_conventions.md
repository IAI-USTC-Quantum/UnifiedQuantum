...
* **跨平台一致性**：OriginQ、Quafu、IBM/Qiskit、Quark 在底层各有差异（IBM 的 BitArray
  以 `c[0]` 为整数最低位、Quafu 在某些 backend 把 `c[0]` 放在最左、Quark 取决于
  固件），但 `uniqc` 的 normalizer 已经统一改写为上述 cbit 框架。任何依赖原始平台
  顺序的下游代码请改用 normalizer 输出。

* **永久自检脚本**：在任何后端（云 / dummy / 本地仿真）跑下面这段电路，dominant
  outcome 都 **必须** 是 `'01'`：
  ...
  print(query_task(uid).result)  # -> {'01': 1024}
  ```

  `x(0)` 把 `q[0]` 翻成 `|1⟩`，第一次 `measure(0)` 写到 `c[0]=1`；按右起 LSB 约定
  bitstring 必为 `'01'`。任何后端给出 `'10'` 即视为 endianness 退化 bug，应优先
  修复 adapter / normalizer 而非业务代码。
