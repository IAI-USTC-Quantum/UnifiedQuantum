...
class BaseSimulator:
    ...
    by specific simulators (e.g., Simulator, Simulator).
    """
    ...
    def _check_available_qubits(self):
        ...
        for used_qubit in used_qubits:
            if used_qubit not in self.available_qubits:
                raise TopologyError(
                    f"A invalid qubit is used. Available qubits: {self.available_qubits}\nUsed: {used_qubit}."
                )
        ...
        for i, opcode in enumerate(program_body):
            ...
            if self.available_topology and isinstance(qubit, list):
                try:
                    ...
                    raise TopologyError(f"Error in line {i + 1}: Opcode: {opcode}, Original error: {exc}") from exc
    ...
    def simulate_pmeasure(self, quantum_code):
        ...
        prob_list = self.opcode_simulator.simulate_opcodes_pmeasure(self.qubit_num, program_body, measure_qubit)
        return prob_list
    ...
    def simulate_statevector(self, quantum_code):
        ...
        """
        program_body, measure_qubit = self.simulate_preprocess(quantum_code)
        statevector = self.opcode_simulator.simulate_opcodes_statevector(self.qubit_num, program_body)
        return statevector
    ...
    def simulate_stateprob(self, quantum_code):
        ...
        """
        program_body, measure_qubit = self.simulate_preprocess(quantum_code)
        stateprob = self.opcode_simulator.simulate_opcodes_stateprob(self.qubit_num, program_body)
        return stateprob
    ...
    def simulate_density_matrix(self, quantum_code):
        ...
        """
        program_body, measure_qubit = self.simulate_preprocess(quantum_code)
        density_matrix = self.opcode_simulator.simulate_opcodes_density_operator(self.qubit_num, program_body)
    ...
    def simulate_single_shot(self, quantum_code):
        ...
        result = self.opcode_simulator.simulate_opcodes_shot(self.qubit_num, processed_program_body, measure_qubit)
        return result
...
class BaseNoisySimulator(BaseSimulator):
    ...
    def simulate_density_matrix(self, originir):
        ...
        if self.opcode_simulator.simulator_typestr == "density_operator":
            ...
            raise ValueError(
                "simulate_density_matrix is only available for density_operator type OpcodeSimulator backend."
            )
