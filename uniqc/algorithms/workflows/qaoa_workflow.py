...
def run_qaoa_workflow(
    ...
    *,
    ...
    p: int = 1,
    init_gammas: np.ndarray | None = None,
    ...
    if len(init_gammas) != p or len(init_betas) != p:
        raise ValueError(f"init_gammas / init_betas must have length p={p}")
    ...
    if not indexed_cost:
        raise ValueError("cost_hamiltonian must contain at least one non-identity term")
    ...
    x0 = np.concatenate([np.asarray(init_gammas, dtype=float), np.asarray(init_betas, dtype=float)])
