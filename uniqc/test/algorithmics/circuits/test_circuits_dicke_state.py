...
class TestDickeStateCircuit:
    def test_dicke_3_1(self):
        ...
        c = dicke_state_circuit(3, k=1, qubits=[0, 1, 2])
        probs = _simulate(c, 3)
    ...
    def test_dicke_k_exceeds_n_raises(self):
        ...
        with pytest.raises(ValueError):
            dicke_state_circuit(3, k=4, qubits=[0, 1, 2])

    def test_dicke_k_negative_raises(self):
        """Negative k should raise ValueError."""
        with pytest.raises(ValueError):
            dicke_state_circuit(2, k=-1, qubits=[0, 1])

    def test_dicke_2_1(self):
        """D(2,1) = (|10⟩+|01⟩)/√2."""
        c = dicke_state_circuit(2, k=1, qubits=[0, 1])
        probs = _simulate(c, 2)
    ...
    def test_dicke_5_3_exact(self):
        ...
        """
        c = dicke_state_circuit(5, k=3, qubits=[0, 1, 2, 3, 4])
        probs = _simulate(c, 5)
    ...
    def test_dicke_6_2_exact(self):
        ...
        """
        c = dicke_state_circuit(6, k=2, qubits=[0, 1, 2, 3, 4, 5])
        probs = _simulate(c, 6)
