...
def simulate_by_uniq(qasm_str, backend_type="statevector", shots=10000):
    """Simulate the circuit by uniqc simulator.
...
def compare_counts(counts1, counts2):
    ...
    if any(np.abs(v) > threshold for v in counts_difference.values()):
        raise NotMatchError(
            f"Counts are not the same.\ncounts1: {counts1}\ncounts2: {counts2}\ncounts_difference: {counts_difference}"
        )


def compare_shots(circuit, backend_type="statevector"):
    # This test compares two density operators generated by qiskit and UnifiedQuantum.
    # The test is based on the following steps:
    # 1. Simulate the circuit using qiskit and UnifiedQuantum.
    # 2. Compare the results.
    # 3. If the results are not the same, raise an error.

    count1 = simulate_by_qiskit(circuit)
    count2 = simulate_by_uniq(circuit, backend_type=backend_type)

    return compare_counts(count1, count2)


def _run_test_random_qasm_compare_shots_impl(
    random_batchsize=100, n_qubit=5, n_gates=20, backend_type="statevector", instruction_set=available_qasm_gates
):
...
def run_test_random_qasm_compare_shots():
    ...
    _run_test_random_qasm_compare_shots_impl(
        random_batchsize=10, n_qubit=5, n_gates=50, instruction_set=gate_set, backend_type="statevector"
    )

    _run_test_random_qasm_compare_shots_impl(
        random_batchsize=10, n_qubit=5, n_gates=50, instruction_set=gate_set, backend_type="density_matrix"
    )

    _run_test_random_qasm_compare_shots_impl(
        random_batchsize=10, n_qubit=5, n_gates=50, instruction_set=gate_set, backend_type="density_matrix_qutip"
    )
